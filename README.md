...
By executing the script file, the user can build an executable of *HLS* in the repository root directory. Please note that this script should be run on a 64-bit GNU/Linux OS. *HLS* is cross-platform; for other OS, please follow the *Makefile* in the *src* directory to build it.
...
Running this command will call the `HLS` program, using `0` as a random seed and set the hyperparameters $\beta=5$, $\alpha=5$, $\varphi=500$, map `./benchmarks/RevLib/mod5mils_65.qasm` quantum circuit on the `IBM Tokyo` device, and store the resulting output in the `./` directory. For this example, the result file name will be `mod5mils_65.qasm_log.txt`.
...
- analytical: provides analytical experiment results mentioned in the Overview section.
...
- hyperpara: provides hyper-parameter analysis results shown in the Appendix.
...
As mentioned in the paper, the main competitors of *HLS* are *EffectiveQM*, *ILS*, *Qiskit*, *LightSABRE*, and *Tket*. Their codes can be obtained from the following link.
...
*LightSABRE* is integrated into *Qiskit*, and its related code is in https://github.com/Qiskit/qiskit/blob/main/qiskit/transpiler/passes/routing/sabre_swap.py.
